...
PPs <- c(1, 2, 5, 10, 30, 50, 100, 1000, 5000) #Individual propagule pressure
...
for (r in 1:nrow(ships_data))  { 
  for (nspec in CPs) {  #Community pressure, number of species
    for (PP in PPs)  { #number of organisms per species
      for (i in 1:iterations) {
        ...
        # Set PP for each species
        max_total_PP<- discharge_volume*PP #max number of organisms (volume * number of species per cubic meter of water )
        potential_pool<- rep(1:nspec,times=(max_total_PP-1)) # minus 1 because every species must be present at least once so 
        species_pool<-c(1:nspec,sample(potential_pool,size=max_total_PP-nspec,replace=FALSE))
        ...
        # Randomly distributed remaining propagules amongst species 
  ...
  print(paste("Ship",r,"finished")) 
}


write.csv(risk_atleast, "CommPropPressure_Risk_AtleastOneSpp_Establishing.csv", row.names = FALSE)
...
risk_exactlyone_prob <- risk_exactlyone %>% 
  mutate(Group = case_when(n_SppEst == 0 ~ "None",
                           n_SppEst == 1 ~ "One",
                           n_SppEst > 1 ~ "More")) %>% 
  group_by(Discharge_volume, CommunityPP, ColonizationP, Group) %>% 
  summarise(NSPPEST_Trip = n()) %>% 
  mutate(Prob = NSPPEST_Trip / iterations) %>% 
  filter(Group == "One")

write.csv(risk_exactlyone_prob , "CommPropPressure_Risk_ExactlyOneSpp_Establishing.csv", row.names = FALSE)
